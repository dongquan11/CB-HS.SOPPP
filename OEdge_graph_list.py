...
m=100     # number of troops of LEARNER
p = 2     # number of troops of ADVERSARY
Time_range = [1000]
save = 'n=3_m=2_test2.csv'

#temp_val= np.random.uniform(low=1, high=8, size=n)
#values= temp_val/sum(temp_val)
values = np.array([0.350458,0.445823,0.203719])
exploration = 'Uniform'     # Either 'Uniform' exploration Or 'Optimal' exploration
setting = 'Bandit'  # Choose setting = 'Full_info' or 'Semi_bandit' or 'Bandit'
for adv_distribution in ['Uniform_1']:    # Adversary plays 'Battlefield_wise' or 'Uniform_1' or 'Test_extreme'
    ...
    # GENERATE values of battlefields
        ...
        #values = np.array([0.85,0.05,0.05,0.05])
        #values = np.array([0.1,0.4,0.5])
        #values= np.repeat(1/n,n)
        #print(values)
        # BLOTTO rule
    def blotto(x,y):
        if (x > y):  return 1
    ...
    def adversary(option):
        ...
        if (option == 'Uniform_1'):
            ...
            adver_stra[0] = m-1    #The first and last battlefield is allocated m by adversary
            # The best (and only) option for learning is putting m troops to first or last battlefield
    ...
    def explore(option):    
        if option == 'Uniform':
            ...
            item = np.random.choice(range(P), size=1, p = distri_mu)
            path = paths_by_nodes[int(item)]
        return path
    ...
    def update_H(w):
        ...
        for j in np.flip(nodes,axis=0):
            for i in np.flip(list(nx.ancestors(G,j)), axis=0):
    ...
    def exploit(H,w):
        ...
        while (len(chosen_path) <= n):
            ...
            for k in nx.dfs_successors(G,node_k_1)[node_k_1]:
                prob = np.append(prob, [w[edges.index((node_k_1,k))] * H[k,N-1] / H[node_k_1,N-1] ])
    #        print(prob)
            node_k = np.asscalar(np.random.choice(nx.dfs_successors(G,node_k_1)[node_k_1], size =1, p =prob  ))
    #        print(node_k)
    ...
    def coocurence_mat(w,H):
        ...
        for e_1 in range(E):
            ...
            for e_2 in range(e_1+1,E):
                mat[e_1,e_2] = H[0, edges[e_1][0]] * w[e_1] * H[edges[e_1][1],edges[e_2][0]] *w[e_2] * H[edges[e_2][1],N-1] / H[0,N-1]
                mat[e_2,e_1] = mat[e_1,e_2]                
        return mat
    
    #print(coocurence_mat(w_test,H))
    ...
    #            elif (estimate[e]  >1): estimate[e] =1
        elif option == 'Semi_bandit':   #################### UNFINISHED ####################
            observe = Loss(adver_stra)*bin_path(path)       # Semi_bandit feedback, only maintain Loss of chosen edged, others set to 0 
            estimate = observe*Loss(adver_stra)
        else:
            print('error in the name of bandit setting')
        return estimate
        
    ##
    #print(est_loss(chosen_path, adver_stra,w_test,'Full_info')  )
    #print(est_loss(chosen_path, adver_stra,w_test,'Semi_bandit')  )
    #print(est_loss(chosen_path, adver_stra,w_test,'Bandit')  )
    ...
    for Time in Time_range :
        ...
        if exploration == 'Uniform':
            ...
            Location = r'C:\Users\qvu\Google Drive\2018 Works\8. AAAI submission\Implementation\distri_mu.csv' #Change this for the location of data file
        ...
        B = np.sqrt(n)
        eta = (1/B)*np.sqrt(  np.log(P) / (T*( (E/(B**2)) + (2/lambda_min) ))  )        #Each action is an (0,1) vectors with dim=E and exactly n numbers 1
        ...
        #    Coin = 0
            ...
            if Coin == 0:       # EXPLOIT 
                ...
                print(estimate)
        #        eta=(lambda_min)/(2*B*B*((t+1)**(1/2)))  #Sasuake decreasing parameter
        #        print(eta)
        ...
        df1 =pd.DataFrame(np.append([n,m,p,T], [gamma, eta, exploration,lambda_min, setting, adv_distribution, end-start, V[m,n], regret]))
